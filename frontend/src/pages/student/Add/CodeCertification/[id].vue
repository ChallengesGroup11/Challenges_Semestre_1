      ...
      <q-card class="q-ma-lg">
        <div class="q-pa-md">
          ...
          <q-form @submit="onSubmit(id)" class="q-gutter-md">
            <q-file @update:model-value="val => { file = val[0] }" filled label="Ajouter votre code de la route"
              type="file" v-model="user.filePathCode">
              <template v-slot:append>
                <q-icon name="attach_file" />
              </template>
            </q-file>
            <q-file @update:model-value="val => { file = val[0] }" filled label="Ajouter votre carte d'identité"
              type="file" v-model="user.filePathCni">
              <template v-slot:append>
                <q-icon name="attach_file" />
              </template>
            </q-file>
