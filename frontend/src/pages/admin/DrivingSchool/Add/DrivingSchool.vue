    ...
    try {
      ...
      if (validDocs.status == 200) {
        ...
        if (response.ok) {
          ...
          viewNotif('warning', 'red', 'Vos documents n\'ont pas été enregistrer, veuillez réessayer', 'white', 'top-right')
        }
      } else {
      ...
      <div class="q-pa-md">
        ...
        <q-form @submit.prevent="onSubmit" class="q-gutter-md">
          <q-input label="Nom de l'école de conduite" filled lazy-rules
            :rules="[val => val.length > 0 || 'Veuillez saisir un nom']" v-model="drive.name" />
          ...
          <q-input label="Code postal" filled lazy-rules
            :rules="[val => val.length > 0 || 'Veuillez saisir un code postal']" v-model="drive.zipcode" />
          ...
          <q-input label="Numéro de téléphone" filled lazy-rules
            :rules="[val => val.length > 0 || 'Veuillez saisir un numéro de téléphone']" v-model="drive.phoneNumber" />
          <q-input label="Numéro de SIRET" filled lazy-rules
            :rules="[val => val.length > 0 || 'Veuillez saisir un numéro de SIRET']" v-model="drive.siret" />
          <q-file @update:model-value="(val) => { file = val[0] }" filled type="file" v-model="drive.filePath" />
